...
function connect(uid){
	...
	socket.emit('webrtc-connection-req', JSON.stringify({"uid": user_id, "ruid": uid}));	
}
...
socket.on('webrtc-connection', function(data){
	...
	weblog("connection request from "+peer_id);
		...
		if(evt.candidate){
			...
			//trace("sending: " +JSON.stringify(pc.localDescription));
	    		// send the offer to the other peer
			socket.emit('webrtc-dsp', JSON.stringify({"uid" : user_id, "ruid" : peer_id, "webrtc": pc.localDescription}));
		}
    	};
		...
		.then(function () {
			// populate offer with candidates from onicecandidate then send offer
...
socket.on('webrtc-message', function(data){
	...
	pc = peers[ruid]["pc"];
	    ...
	    peers[ruid] = {"pc":pc, "channel":channel};
    ...
    if (message.type) {
	    ...
	    let desc = new RTCSessionDescription(message);
		 ...
		 pc.onicecandidate = function(evt){
                        if(evt.candidate){
                                //trace(JSON.stringify(evt.candidate));
                                let candidate = new RTCIceCandidate(evt.candidate);
                                pc.addIceCandidate(candidate);
                        }else{
...
function handleChannelServer(channel, peer_id) {
	...
	switch(msg.type){
		case "file-list":
			...
			channel.send(JSON.stringify({"uid":user_id, "type":"file-list-rep", "data":file_array}));					     break;
		case "file-request":
			//alert(msg["file-name"]);
...
function requestFile(t, peer_id){
	...
	channel = peers[peer_id]["channel"];
        ...
        channel.send(JSON.stringify({"uid":user_id, "type":"file-request", "file-name":file_name}));
}
...
function sendData(file_name, channel){
	...
	channel.send(JSON.stringify({"uid": user_id, "type":"file-rep", "data": {"name":file.name, "size":file.size, "type":file.type}}));
	var chunkSize = 16384;
